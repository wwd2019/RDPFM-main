...
# is_datasets_available函数用于检查当前环境是否可用datasets库，该库提供了许多用于自然语言处理任务的数据集。如果返回True，则表示datasets库可用；如果返回False，则表示datasets库不可用。
#
# is_sagemaker_dp_enabled函数用于检查当前环境是否启用了SageMaker分布式训练。SageMaker是亚马逊AWS提供的一项机器学习服务，分布式训练是其中的一项功能。如果返回True，则表示启用了SageMaker分布式训练；如果返回False，则表示未启用SageMaker分布式训练
...
class TruncationStrategy(ExplicitEnum):
    ...
    在PreTrainedTokenizerBase类的__call__方法中，truncation参数的可能取值有以下几种：

    “longest_first”：将输入文本进行截断，保留最长的部分。如果输入文本长度超过了模型的最大输入长度，则会从开头截断。
    “only_first”：只保留输入文本的前部分，截断后面的部分。
    “only_second”：只保留输入文本的后部分，截断前面的部分。
    False：不进行截断，保留完整的输入文本
...
“longest”: 将输入序列填充为最长的序列长度。这意味着所有的输入序列都会被填充为与最长序列相同的长度。

“max_length”: 将输入序列填充为指定的最大长度。如果输入序列长度超过了最大长度，则会被截断为最大长度。

“do_not_pad”: 不进行填充操作，保持输入序列的原始长度。

“True”: 与"longest"相同，将输入序列填充为最长的序列长度。

“False”: 与"do_not_pad"相同，不进行填充操作
    """
...
class GeneformerPreCollator(SpecialTokensMixin):
    # kwargs是Python中的一个特殊参数，它允许函数接受任意数量的关键字参数。kwargs是一个字典，其中关键字是参数名，对应的值是传递给函数的实际参数值。
    ...
    def convert_ids_to_tokens(self,value):
        # 将input_ids转为对应的token
        return (self.token_dictionary.get
                (value))
    ...
    def _get_padding_truncation_strategies(
        ...
        **kwargs,
        ...
        在处理旧的参数（truncation_strategy和pad_to_max_length）和行为的情况下，找到正确的填充/截断策略并保持向后兼容性
        """
...
def get_length_grouped_indices(
    ...
    返回一个索引列表，使得每个大小为batch_size的连续索引片段对应于长度相似的元素。具体实现方式如下：

    首先，将索引随机打乱。
    然后，将索引按照mega_batch_mult * batch_size的大小分组成mega-batches。
    在每个mega-batch中，按照元素的长度进行排序。
    最后，将所有mega-batches的索引连接起来，其中大小为batch_size的批次中包含的最长元素被放在第一个位置，这样可以更早地发生OOM（内存溢出）。
