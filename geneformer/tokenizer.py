...
“ensembl_id”：每个基因的Ensembl ID。Ensembl是一个基因组注释数据库，它为每个基因提供唯一的标识符。

“n_counts”：每个细胞中的总读数。这个属性表示在每个细胞中检测到的总RNA分子数量，可以用来衡量细胞的RNA表达水平。

“filter_pass”：基于用户定义的筛选条件，对细胞进行标记的二进制指示器。这个属性可以用来标记细胞是否通过了用户定义的筛选条件。

其他细胞元数据：除了上述属性之外，还可以传递任何其他细胞元数据作为自定义属性字典。这些属性可以包括细胞类型、实验条件、样本来源等信息，以便在后续的数据处理和分析中使用。
